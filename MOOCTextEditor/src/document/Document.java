...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		// Count number of instances of sequences of one or more vowels
		String pattern = "aeiouyAEIOUY";
		int numSyllables = 0;
		boolean newSyllable = true;
		for (char c : word.toCharArray()){
			if (checkCharInString(pattern, c) && (newSyllable)) {
				numSyllables ++;
				newSyllable = false;
			}
			else if (!checkCharInString(pattern, c)) {
				newSyllable = true;
			}
		}

		// Consider the edge case of a lone 'e' at the end of the word,
		// where there are already more than one syllables.
		String ePattern = "eE";
		int n = word.length();
		char last = word.charAt(n-1);
		if ((n>1) && checkCharInString(ePattern, last)
				&& (numSyllables > 1)) {
			if (!checkCharInString(pattern, word.charAt(n - 2))) {
				numSyllables--;
			}
		}
//		System.out.println(word + " : " + numSyllables);
	...
	protected boolean checkCharInString(String s, char c){
