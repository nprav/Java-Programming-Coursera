...
//		System.out.println(tokens);
		...
		for (String tok : tokens) {
			// update numWords and numSyllables
			if (isWord(tok)) {
				numWords ++;
				numSyllables += countSyllables(tok);
			}
			else {
				numSentences ++;	// captures all sentences that end in a punctuation mark
			}
		}
		// Add a sentence if final token is not a punctuation mark
